...
const SLUG = "openclaw-raspberry-pi-5";
const FEATURED_IMAGE_URL = "/images/blog/c11-openclaw-raspberry-pi-5.webp";
...
function loadLocalizedPostsFromMarkdown(): Record<SupportedLocale, LocalizedPost> {
  const baseDir = path.join(process.cwd(), "data", "cms", "c11");
...
async function main() {
  try {
    ...
    console.error("seed-c11 failed:", error);
    process.exitCode = 1;
  } finally {
    await sql.end();
  }
