...
const SLUG = "c07-snag-screenshot-to-markdown";
const FEATURED_IMAGE_URL = "/images/blog/c07-snag-screenshot-to-markdown.webp";
...
function loadLocalizedPostsFromMarkdown(): Record<SupportedLocale, LocalizedPost> {
  const baseDir = path.join(process.cwd(), "data", "cms", "c07");
...
async function main() {
  try {
    ...
    console.error("seed-c07-user-case-snag failed:", error);
    process.exitCode = 1;
  } finally {
    await sql.end();
  }
