...
const SLUG = "voice-assistant-openclaw";
const FEATURED_IMAGE_URL = "/images/blog/c13-voice-assistant-openclaw.webp";
...
function loadLocalizedPostsFromMarkdown(): Record<SupportedLocale, LocalizedPost> {
  const baseDir = path.join(process.cwd(), "data", "cms", "c13");
...
async function main() {
  try {
    ...
    console.error("seed-c13 failed:", error);
    process.exitCode = 1;
  } finally {
    await sql.end();
  }
