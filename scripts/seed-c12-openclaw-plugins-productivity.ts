...
const SLUG = "openclaw-plugins-productivity";
const FEATURED_IMAGE_URL = "/images/blog/c12-openclaw-plugins-productivity.webp";
...
function loadLocalizedPostsFromMarkdown(): Record<SupportedLocale, LocalizedPost> {
  const baseDir = path.join(process.cwd(), "data", "cms", "c12");
...
async function main() {
  try {
    ...
    console.error("seed-c12 failed:", error);
    process.exitCode = 1;
  } finally {
    await sql.end();
  }
