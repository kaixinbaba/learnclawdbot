...
const SLUG = "openclaw-multi-user-teams";
const FEATURED_IMAGE_URL = "/images/blog/c16-openclaw-multi-user-teams.webp";
...
function loadLocalizedPostsFromMarkdown(): Record<SupportedLocale, LocalizedPost> {
  const baseDir = path.join(process.cwd(), "data", "cms", "c16");
...
async function main() {
  try {
    ...
    console.error("seed-c16 failed:", error);
    process.exitCode = 1;
  } finally {
    await sql.end();
  }
