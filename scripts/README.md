...
### seed-c04-user-case-bambu-cli.ts

写入 C04 用户案例（bambu-cli + OpenClaw 的 BambuLab 3D 打印控制流程）到 CMS 数据库（`posts` / `tags` / `post_tags`），包含 5 语言内容与标签关联，支持幂等重复执行。
...
# 或直接执行
npx tsx scripts/seed-c04-user-case-bambu-cli.ts
```

**用途：**
- 以 CMS 流程发布 C04（不走 mdx 静态文件）
- 自动确保标签存在并关联
...
```bash
# 推荐（package script）
pnpm cms:migrate:featured-webp

# 或直接执行
npx tsx scripts/migrate-blog-featured-images-to-webp.ts
```

**用途：**
- 修复历史文章 `featured_image_url` 的旧格式引用
- 与 `/public/images/blog/*.webp` 头图资源保持一致
- 输出剩余 legacy 引用，便于回归检查
