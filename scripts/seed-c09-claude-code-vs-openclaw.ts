...
const SLUG = "claude-code-vs-openclaw";
const FEATURED_IMAGE_URL = "/images/blog/c09-claude-code-vs-openclaw.webp";
...
function loadLocalizedPostsFromMarkdown(): Record<SupportedLocale, LocalizedPost> {
  const baseDir = path.join(process.cwd(), "data", "cms", "c09");
...
async function main() {
  try {
    ...
    console.error("seed-c09 failed:", error);
    process.exitCode = 1;
  } finally {
    await sql.end();
  }
