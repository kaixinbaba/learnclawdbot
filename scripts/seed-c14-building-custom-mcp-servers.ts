...
const SLUG = "building-custom-mcp-servers-openclaw";
const FEATURED_IMAGE_URL = "/images/blog/c14-building-custom-mcp-servers.webp";
...
function loadLocalizedPostsFromMarkdown(): Record<SupportedLocale, LocalizedPost> {
  const baseDir = path.join(process.cwd(), "data", "cms", "c14");
...
async function main() {
  try {
    ...
    console.error("seed-c14 failed:", error);
    process.exitCode = 1;
  } finally {
    await sql.end();
  }
