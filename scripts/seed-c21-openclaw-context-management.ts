...
const SLUG = "openclaw-context-management-optimization";
const FEATURED_IMAGE_URL = "/images/blog/c21-openclaw-context-management.webp";
...
function loadLocalizedPostsFromMarkdown(): Record<SupportedLocale, LocalizedPost> {
  const baseDir = path.join(process.cwd(), "data", "cms", "c21");
  ...
  for (const locale of LOCALES) {
    ...
    if (!description) throw new Error(`Missing frontmatter description in ${filePath}`);

    posts[locale] = { title, description, content: content.trim() };
  }
...
async function ensureTags() {
  ...
  for (const tagName of TAG_NAMES) {
    const result = await sql`
      ...
      ON CONFLICT (name, post_type) DO UPDATE SET name = EXCLUDED.name
      RETURNING id, name;
    `;
...
async function upsertLocalizedPosts(
  ...
  const summary = { inserted: 0, updated: 0, locales: [] as SupportedLocale[] };

  for (const locale of LOCALES) {
    const post = localizedPosts[locale];
    const existing = await sql`
      SELECT id FROM posts WHERE slug = ${SLUG} AND language = ${locale} AND post_type = ${POST_TYPE} LIMIT 1;
    `;

    const result = await sql`
      INSERT INTO posts (language, post_type, author_id, title, slug, content, description, featured_image_url, is_pinned, status, visibility, published_at)
      VALUES (${locale}, ${POST_TYPE}, ${authorId}, ${post.title}, ${SLUG}, ${post.content}, ${post.description}, ${FEATURED_IMAGE_URL}, false, 'published', 'public', NOW())
      ON CONFLICT (slug, language, post_type) DO UPDATE SET
        title = EXCLUDED.title, content = EXCLUDED.content, description = EXCLUDED.description,
        featured_image_url = EXCLUDED.featured_image_url, status = EXCLUDED.status,
    ...
    for (const tagId of tagIds) {
      await sql`INSERT INTO post_tags (post_id, tag_id) VALUES (${postId}, ${tagId}) ON CONFLICT (post_id, tag_id) DO NOTHING;`;
    }

    if (existing.length > 0) summary.updated += 1;
...
async function main() {
  try {
    ...
    const postSummary = await upsertLocalizedPosts(localizedPosts, authorId, tagIds);
    console.log(JSON.stringify({ ok: true, slug: SLUG, featuredImageUrl: FEATURED_IMAGE_URL, tags: TAG_NAMES, postSummary }, null, 2));
  } catch (error) {
    console.error("seed-c21 failed:", error);
    process.exitCode = 1;
  } finally {
    await sql.end();
  }
