...
const SLUG = "openclaw-roadmap-2026";
const FEATURED_IMAGE_URL = "/images/blog/c22-openclaw-roadmap-2026.webp";
...
function loadLocalizedPostsFromMarkdown(): Record<SupportedLocale, LocalizedPost> {
  const baseDir = path.join(process.cwd(), "data", "cms", "c22");
...
async function main() {
  try {
    ...
    console.error("seed-c22 failed:", error);
    process.exitCode = 1;
  } finally {
    await sql.end();
  }
