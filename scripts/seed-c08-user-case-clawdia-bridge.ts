...
const SLUG = "clawdia-phone-bridge";
const FEATURED_IMAGE_URL = "/images/features/clawdia-phone-bridge.webp";
...
function loadLocalizedPostsFromMarkdown(): Record<SupportedLocale, LocalizedPost> {
  const baseDir = path.join(process.cwd(), "data", "cms", "c08");
...
async function main() {
  try {
    ...
    console.error("seed-c08-user-case-clawdia-bridge failed:", error);
    process.exitCode = 1;
  } finally {
    await sql.end();
  }
