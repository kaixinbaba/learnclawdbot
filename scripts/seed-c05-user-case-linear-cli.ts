...
const SLUG = "c05-linear-cli-openclaw-issue-workflow";
const FEATURED_IMAGE_URL = "/images/blog/c05-linear-cli-issue-workflow.webp";
...
function loadLocalizedPostsFromMarkdown(): Record<SupportedLocale, LocalizedPost> {
  const baseDir = path.join(process.cwd(), "data", "cms", "c05");
...
async function main() {
  try {
    ...
    console.error("seed-c05-user-case-linear-cli failed:", error);
    process.exitCode = 1;
  } finally {
    await sql.end();
  }
