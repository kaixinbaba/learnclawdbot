...
const SLUG = "openclaw-vs-autogpt-comparison";
const FEATURED_IMAGE_URL = "/images/blog/c23-openclaw-vs-autogpt.webp";
...
function loadLocalizedPostsFromMarkdown(): Record<SupportedLocale, LocalizedPost> {
  const baseDir = path.join(process.cwd(), "data", "cms", "c23");
...
async function main() {
  try {
    ...
    console.error("seed-c23 failed:", error);
    process.exitCode = 1;
  } finally {
    await sql.end();
  }
