...
const SLUG = "hardening-openclaw-security";
const FEATURED_IMAGE_URL = "/images/blog/c15-hardening-openclaw-security.webp";
...
function loadLocalizedPostsFromMarkdown(): Record<SupportedLocale, LocalizedPost> {
  const baseDir = path.join(process.cwd(), "data", "cms", "c15");
...
async function main() {
  try {
    ...
    console.error("seed-c15 failed:", error);
    process.exitCode = 1;
  } finally {
    await sql.end();
  }
