...
const SLUG = "openclaw-deepseek-low-cost";
const FEATURED_IMAGE_URL = "/images/blog/c10-openclaw-deepseek-low-cost.webp";
...
function loadLocalizedPostsFromMarkdown(): Record<SupportedLocale, LocalizedPost> {
  const baseDir = path.join(process.cwd(), "data", "cms", "c10");
...
async function main() {
  try {
    ...
    console.error("seed-c10 failed:", error);
    process.exitCode = 1;
  } finally {
    await sql.end();
  }
