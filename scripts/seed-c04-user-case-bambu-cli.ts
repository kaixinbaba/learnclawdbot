...
const SLUG = "c04-bambu-cli-3d-printer-control";
const FEATURED_IMAGE_URL = "/images/blog/c04-bambu-cli-3d-printer-control.webp";
...
function loadLocalizedPostsFromMarkdown(): Record<SupportedLocale, LocalizedPost> {
  const baseDir = path.join(process.cwd(), "data", "cms", "c04");
...
async function main() {
  try {
    ...
    console.error("seed-c04-user-case-bambu-cli failed:", error);
    process.exitCode = 1;
  } finally {
    await sql.end();
  }
