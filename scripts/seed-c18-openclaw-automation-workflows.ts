...
const SLUG = "openclaw-automation-workflows";
const FEATURED_IMAGE_URL = "/images/blog/c18-openclaw-automation-workflows.webp";
...
function loadLocalizedPostsFromMarkdown(): Record<SupportedLocale, LocalizedPost> {
  const baseDir = path.join(process.cwd(), "data", "cms", "c18");
...
async function main() {
  try {
    ...
    console.error("seed-c18 failed:", error);
    process.exitCode = 1;
  } finally {
    await sql.end();
  }
