...
const SLUG = "openclaw-ollama-offline-privacy";
const FEATURED_IMAGE_URL = "/images/blog/c19-openclaw-ollama-offline.webp";
...
function loadLocalizedPostsFromMarkdown(): Record<SupportedLocale, LocalizedPost> {
  const baseDir = path.join(process.cwd(), "data", "cms", "c19");
...
async function main() {
  try {
    ...
    console.error("seed-c19 failed:", error);
    process.exitCode = 1;
  } finally {
    await sql.end();
  }
