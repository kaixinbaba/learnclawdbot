...
const SLUG = "c06-multi-agent-orchestration-kev-dream-team";
const FEATURED_IMAGE_URL = "/images/blog/c06-multi-agent-orchestration.webp";
...
function loadLocalizedPostsFromMarkdown(): Record<SupportedLocale, LocalizedPost> {
  const baseDir = path.join(process.cwd(), "data", "cms", "c06");
...
async function main() {
  try {
    ...
    console.error("seed-c06-user-case-multi-agent-orchestration failed:", error);
    process.exitCode = 1;
  } finally {
    await sql.end();
  }
