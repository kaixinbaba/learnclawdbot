...
const SLUG = "debugging-openclaw-logs-traces";
const FEATURED_IMAGE_URL = "/images/blog/c17-debugging-openclaw.webp";
...
function loadLocalizedPostsFromMarkdown(): Record<SupportedLocale, LocalizedPost> {
  const baseDir = path.join(process.cwd(), "data", "cms", "c17");
...
async function main() {
  try {
    ...
    console.error("seed-c17 failed:", error);
    process.exitCode = 1;
  } finally {
    await sql.end();
  }
