...
const SLUG = "openclaw-home-assistant-smart-home";
const FEATURED_IMAGE_URL = "/images/blog/c20-openclaw-home-assistant.webp";
...
function loadLocalizedPostsFromMarkdown(): Record<SupportedLocale, LocalizedPost> {
  const baseDir = path.join(process.cwd(), "data", "cms", "c20");
...
async function main() {
  try {
    ...
    console.error("seed-c20 failed:", error);
    process.exitCode = 1;
  } finally {
    await sql.end();
  }
