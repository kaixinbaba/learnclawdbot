...
목표: **Bun**으로 이 저장소를 실행하기 (선택 사항, WhatsApp/Telegram에는 권장하지 않음)
...
- `pnpm`이 빌드의 기본값이며 완전히 지원됩니다(일부 문서 도구에서 사용).
...
참고: `bun.lock`/`bun.lockb`는 gitignore되어 있으므로 어느 쪽이든 저장소 변경은 없습니다. *lockfile 쓰기를 원하지 않으면*:
...
Bun은 명시적으로 신뢰하지 않는 한(`bun pm untrusted` / `bun pm trust`) 의존성 라이프사이클 스크립트를 차단할 수 있습니다.
