---
summary: "WKWebView와 커스텀 URL 스킴으로 임베드된 에이전트 제어 Canvas 패널"
read_when:
  - macOS Canvas 패널 구현 시
  - 비주얼 워크스페이스를 위한 에이전트 컨트롤 추가 시
  - WKWebView canvas 로드 디버깅 시
---
# Canvas (macOS 앱)

macOS 앱은 `WKWebView`를 사용하여 에이전트가 제어하는 **Canvas 패널**을 내장합니다. 이는
HTML/CSS/JS, A2UI, 작은 인터랙티브 UI 표면을 위한 경량 비주얼 워크스페이스입니다.
...
Canvas 패널은 **커스텀 URL 스킴**을 통해 해당 파일들을 제공합니다:
...
- 세션별로 크기/위치를 기억합니다.
- 로컬 canvas 파일이 변경되면 자동으로 리로드됩니다.
- 한 번에 하나의 Canvas 패널만 표시됩니다 (필요에 따라 세션 전환).

Settings → **Allow Canvas**에서 Canvas를 비활성화할 수 있습니다. 비활성화되면, canvas
노드 명령은 `CANVAS_DISABLED`를 반환합니다.
...
A2UI는 Gateway canvas 호스트에 의해 호스팅되며 Canvas 패널 내부에 렌더링됩니다.
Gateway가 Canvas 호스트를 광고하면, macOS 앱은 첫 번째 열기 시 A2UI 호스트 페이지로
자동으로 이동합니다.
