...
read_when:
  ...
  - OpenClaw를 위한 저비용 VPS 호스팅을 찾고 있을 때
...
이것은 Tailscale SSH를 활성화하여 tailnet의 모든 기기에서 `ssh openclaw`로 연결할 수 있습니다 — 공개 IP 불필요.
...
이 설정은 종종 인터넷 전체의 SSH 무차별 대입 공격을 막기 위한 추가 호스트 기반 방화벽 규칙의 *필요성을 제거*합니다 — 그러나 여전히 OS를 업데이트하고, `openclaw security audit`를 실행하고, 실수로 공개 인터페이스에서 수신 대기하지 않는지 확인해야 합니다.
