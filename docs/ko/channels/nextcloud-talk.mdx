...
구성/온보딩 중에 Nextcloud Talk를 선택하고 git 체크아웃이 감지되면,
...
- Webhook 페이로드는 DM과 방을 구별하지 않습니다. 방 유형 조회를 활성화하려면 `apiUser` + `apiPassword`를 설정하세요(그렇지 않으면 DM이 방으로 처리됨).
...
- `channels.nextcloud-talk.rooms`로 방을 허용목록에 추가합니다:
```json5
{
  channels: {
    "nextcloud-talk": {
...
- `channels.nextcloud-talk.allowFrom`: DM 허용목록 (사용자 ID). `open`은 `"*"`가 필요합니다.
...
- `channels.nextcloud-talk.chunkMode`: `length` (기본값) 또는 `newline`으로 길이 청킹 전에 빈 줄(단락 경계)로 분할.
