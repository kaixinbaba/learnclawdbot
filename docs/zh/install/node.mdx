...
read_when:
  - '你安装了 OpenClaw 但 `openclaw` 显示 "command not found"'
  - "你正在新机器上设置 Node.js/npm"
...
如果你可以运行 `npm install -g openclaw@latest` 但后来看到 `openclaw: command not found`，这几乎总是一个 **PATH** 问题：npm 放置全局二进制文件的目录不在你的 shell 的 PATH 中。
...
- Windows：官方 Node 安装器、`winget` 或 Windows Node 版本管理器

如果你使用版本管理器（nvm/fnm/asdf 等），确保它在你日常使用的 shell 中初始化（zsh vs bash），以便在你运行安装器时它设置的 PATH 存在。
