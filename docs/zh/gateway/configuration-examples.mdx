...
以下示例与当前配置模式对齐。有关详尽的参考和每个字段的说明，请参见 [Configuration](/gateway/configuration)。
...
保存到 `~/.openclaw/openclaw.json`，你就可以从该号码向 bot 发送 DM。
...
```json5
{
  ...
  // 认证配置文件元数据（密钥位于 auth-profiles.json）
  auth: {
    profiles: {
...
- 参见 [Providers](/channels/whatsapp) 和 [Troubleshooting](/gateway/troubleshooting) 以获取更深入的设置说明。
