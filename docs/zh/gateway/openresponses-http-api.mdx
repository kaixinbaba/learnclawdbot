...
默认情况下，端点是**每个请求无状态的**（生成新的会话键）。

如果请求包含 OpenResponses `user` 字符串，Gateway 从中派生稳定的会话键，因此重复调用可以共享 agent 会话。

## 请求形状（支持的）

请求遵循 OpenResponses API，基于项目的输入。当前支持：
...
- `stream`：启用 SSE 流式传输。
- `max_output_tokens`：最佳努力输出限制（取决于提供商）。
- `user`：稳定的会话路由。
...
- 较早的 user/assistant 消息作为历史包含以提供上下文。
...
如果 agent 决定调用工具，响应返回一个 `function_call` 输出项。然后你发送一个带有 `function_call_output` 的后续请求以继续轮次。
...
当前行为：
- 文件内容被解码并添加到**系统提示**，而不是用户消息，因此它保持短暂（不持久化在会话历史中）。
- PDF 被解析为文本。如果找到的文本很少，第一页会被栅格化为图像并传递给模型。
