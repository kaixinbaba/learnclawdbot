---
summary: "TypeBox 架构作为网关协议的单一真相来源"
read_when:
  - 更新协议架构或代码生成
---
# TypeBox 作为协议真相来源
...
TypeBox 是一个 TypeScript 优先的架构库。我们使用它来定义 **Gateway WebSocket 协议**（握手、请求/响应、服务器事件）。这些架构驱动**运行时验证**、**JSON Schema 导出**和用于 macOS 应用的 **Swift 代码生成**。一个真相来源；其他一切都是生成的。

如果你想要更高级别的协议上下文，从 [Gateway architecture](/concepts/architecture) 开始。
...
| 类别 | 示例 | 注意 |
| --- | --- | --- |
| Core（核心） | `connect`, `health`, `status` | `connect` 必须是第一个 |
| Messaging（消息） | `send`, `poll`, `agent`, `agent.wait` | 副作用需要 `idempotencyKey` |
| Chat（聊天） | `chat.history`, `chat.send`, `chat.abort`, `chat.inject` | WebChat 使用这些 |
| Sessions（会话） | `sessions.list`, `sessions.patch`, `sessions.delete` | 会话管理 |
| Nodes（节点） | `node.list`, `node.invoke`, `node.pair.*` | Gateway WS + 节点操作 |
| Events（事件） | `tick`, `presence`, `agent`, `chat`, `health`, `shutdown` | 服务器推送 |
...
- **服务器端**：每个入站帧都使用 AJV 验证。握手只接受参数匹配 `ConnectParams` 的 `connect` 请求。
- **客户端**：JS 客户端在使用事件和响应帧之前验证它们。
- **方法表面**：Gateway 在 `hello-ok` 中宣传支持的 `methods` 和 `events`。
...
- `GatewayFrame` 枚举，带 `req`、`res`、`event` 和 `unknown` 案例
- 强类型负载结构/枚举
- `ErrorCode` 值和 `GATEWAY_PROTOCOL_VERSION`

未知帧类型作为原始负载保留以实现向前兼容性。
...
- 带副作用的方法通常需要在参数中使用 `idempotencyKey`（示例：`send`、`poll`、`agent`、`chat.send`）。

## 实时架构 JSON

生成的 JSON Schema 在仓库的 `dist/protocol.schema.json` 中。发布的原始文件通常可在以下位置获得：
