...
Ollama 是一个本地 LLM 运行时,可以轻松在您的机器上运行开源模型。OpenClaw 集成了 Ollama 的 OpenAI 兼容 API,并且当您使用 `OLLAMA_API_KEY`(或认证配置)选择加入且未定义显式的 `models.providers.ollama` 条目时,可以**自动发现支持工具的模型**。
...
3) 为 OpenClaw 启用 Ollama(任何值都可以;Ollama 不需要真实密钥):
...
当您设置 `OLLAMA_API_KEY`(或认证配置)且**未**定义 `models.providers.ollama` 时,OpenClaw 会从本地 Ollama 实例 `http://127.0.0.1:11434` 发现模型:
...
如果设置了 `OLLAMA_API_KEY`,您可以在 provider 条目中省略 `apiKey`,OpenClaw 会为可用性检查填充它。
...
对于自动发现的模型,OpenClaw 会在可用时使用 Ollama 报告的上下文窗口,否则默认为 `8192`。您可以在显式 provider 配置中覆盖 `contextWindow` 和 `maxTokens`。

## 故障排除

### 未检测到 Ollama

确保 Ollama 正在运行,并且您设置了 `OLLAMA_API_KEY`(或认证配置),并且您**没有**定义显式的 `models.providers.ollama` 条目:
...
OpenClaw 仅自动发现报告工具支持的模型。如果您的模型未列出,可以:
- 拉取一个支持工具的模型,或
