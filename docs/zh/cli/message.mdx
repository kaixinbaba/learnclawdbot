...
用于发送消息和频道操作的单一出站命令（Discord/Google Chat/Slack/Mattermost（插件）/Telegram/WhatsApp/Signal/iMessage/MS Teams）。
...
- `send`
  ...
  - 仅 Telegram：`--buttons`（需要 `channels.telegram.capabilities.inlineButtons` 来允许）
...
- `react`
  ...
  - 注意：`--remove` 需要 `--emoji`（省略 `--emoji` 以清除自己的反应，如果支持；参见 /tools/reactions）
...
- `broadcast`
  - 频道：任何已配置的频道；使用 `--channel all` 来定向所有提供商
  - 必需：`--targets`（重复）
