---
summary: "OpenClaw CLI 参考手册：`openclaw` 命令、子命令和选项"
read_when:
  - 添加或修改 CLI 命令或选项时
  - 记录新的命令接口时
---
...
- `--no-color`：禁用 ANSI 颜色。
- `--update`：`openclaw update` 的快捷方式（仅限源码安装）。
- `-V`, `--version`, `-v`：打印版本并退出。
...
- `openclaw security audit` — 审计配置 + 本地状态的常见安全隐患。
- `openclaw security audit --deep` — 尽力尝试实时 Gateway 探测。
...
### `config`
非交互式配置帮助器（get/set/unset）。不带子命令运行 `openclaw config` 会启动向导。
...
- `channels status`：检查 gateway 可达性和渠道健康（`--probe` 运行额外检查；使用 `openclaw health` 或 `openclaw status --deep` 进行 gateway 健康探测）。
- 提示：`channels status` 在检测到常见错误配置时打印警告和建议修复（然后指向 `openclaw doctor`）。
...
### `dns setup`
广域发现 DNS 帮助器（CoreDNS + Tailscale）。参见 [/gateway/discovery](/gateway/discovery)。

选项：
- `--apply`：安装/更新 CoreDNS 配置（需要 sudo；仅限 macOS）。

## 消息 + 代理（Messaging + agent）
...
### `agent`
通过 Gateway（或 `--local` 嵌入式）运行一个代理回合。
...
绑定规范使用 `channel[:accountId]`。当 WhatsApp 省略 `accountId` 时，使用默认账户 ID。
...
注意：
- 概述包括 Gateway + 节点主机服务状态（如果可用）。

### 使用情况跟踪（Usage tracking）
当 OAuth/API 凭据可用时，OpenClaw 可以显示提供商使用/配额。

界面：
- `/status`（可用时添加简短的提供商使用行）
- `openclaw status --usage`（打印完整提供商细分）
...
- `gateway status` 在检测到旧版或额外的 gateway 服务时也会显示（`--deep` 添加系统级扫描）。以配置文件命名的 OpenClaw 服务被视为一等公民，不会被标记为 "额外"。
...
- `gateway install` 默认使用 Node 运行时；**不推荐** bun（WhatsApp/Telegram 存在 bug）。
...
### `gateway <subcommand>`
Gateway CLI 帮助器（RPC 子命令使用 `--url`、`--token`、`--password`、`--timeout`、`--expect-final`）。
...
完整的回退行为和扫描策略参见 [/concepts/models](/concepts/models)。
...
始终包括认证概述和认证存储中配置文件的 OAuth 过期状态。
...
- `cron add`（别名：`create`；需要 `--name` 和恰好一个 `--at` | `--every` | `--cron`，以及恰好一个有效负载 `--system-event` | `--message`）
- `cron edit <id>`（修补字段）
...
`node` 运行**无头节点主机（headless node host）**或将其作为后台服务管理。参见
...
`nodes` 与 Gateway 通信并针对配对节点。参见 [/nodes](/nodes)。
