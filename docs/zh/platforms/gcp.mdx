---
summary: "在 GCP Compute Engine VM（Docker）上全天候运行 OpenClaw Gateway，配置持久化状态"
read_when:
  - 你想在 GCP 上全天候运行 OpenClaw
  - 你想要一个生产级的、始终在线的 Gateway 在你自己的 VM 上
  - 你想要完全控制持久化、二进制文件和重启行为
---

# OpenClaw 在 GCP Compute Engine（Docker，生产 VPS 指南）

## 目标

使用 Docker 在 GCP Compute Engine VM 上运行持久化的 OpenClaw Gateway（网关），配置持久化状态、内置二进制文件和安全的重启行为。

如果你想要"OpenClaw 全天候运行，约 $5-12/月"，这是 Google Cloud 上的可靠设置。
定价因机器类型和区域而异；选择适合你工作负载的最小 VM，如果遇到 OOM（内存不足）则扩容。
...
在 https://console.cloud.google.com/billing 启用计费（Compute Engine 需要）。
...
| e2-micro | 2 vCPU（共享），1GB RAM | 符合免费套餐资格 | 负载下可能 OOM |
...
services:
  openclaw-gateway:
    ...
    ports:
      # 推荐：在 VM 上保持 Gateway 仅回环；通过 SSH 隧道访问。
...
对于自动化或 CI/CD 流水线，创建具有最小权限的专用服务账户：
