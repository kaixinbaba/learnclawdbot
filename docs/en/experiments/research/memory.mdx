...
summary: "Offline-first memory architecture with Markdown as canonical source and derived index for structured recall"
---
...
1. SQLite FTS5 + metadata filters (zero ML)
2. Embeddings + brute force (works surprisingly far if chunk count is low)
3. HNSW index (common, robust; needs a library binding)
4. SuCo (research-grade; attractive if there's a solid implementation you can embed)
