...
summary: "How OpenClaw manages sessions, transcripts, compaction, and context limits"
---
...
- Transcripts: `~/.openclaw/agents/<agent>/sessions/<sessionId>.jsonl`

Telegram topic sessions: `.../<sessionKey>-topic-<topicId>.jsonl`
...
- Main/direct chat (per agent): `agent:<agent>:<provider>` (default main)
- Group: `agent:<agent>:<provider>:group:<groupId>`
- Room/channel (Discord/Slack): `agent:<agent>:<provider>:channel:<channelId>` or `...:room:<roomId>`
- Cron: `cron:<jobId>`
- Webhook: `hook:<hookId>` (unless overridden)
...
- **Toggles:** `thinkingLevel`, `verboseLevel`, `reasoningLevel`, `elevatedLevel`
...
- If `compaction.reserveTokens < 16384`: raise it to 16384
