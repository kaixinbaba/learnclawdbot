---
title: "RELEASING"
summary: "Release checklist for publishing OpenClaw to npm and building macOS app"
---
...
- Verify the registry: `npm view openclaw version`, `npm view openclaw dist-tags`, and `npx -y openclaw@latest --version` (or `--help`)
...
- From a clean temp directory (no `package.json`), run `npx -y openclaw@latest send --help` to confirm install/CLI entrypoints work
