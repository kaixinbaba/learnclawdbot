...
<figure>
  <img src="/images/integrations/telegram/1.png" alt="App Store 中的 Telegram 应用" />
  <figcaption>在应用商店下载 Telegram，支持 iOS、Android、Windows、macOS 等全平台</figcaption>
</figure>
...
在 Telegram 中搜索 `@BotFather`，或直接点击 [这个链接](https://t.me/BotFather)。

<figure>
  <img src="/images/integrations/telegram/2.png" alt="在 Telegram 中搜索 BotFather" />
  <figcaption>搜索 @BotFather —— 认准官方认证标志（蓝色勾），这是 Telegram 官方的机器人管理工具</figcaption>
</figure>
...
<figure>
  <img src="/images/integrations/telegram/3.png" alt="BotFather 创建 Bot 的完整流程" />
  <figcaption>完整的创建流程：发送 /newbot → 输入显示名称 → 输入用户名 → 获得 Bot Token（红框部分）</figcaption>
</figure>
...
有两种方式把 Token 传给 OpenClaw：**QuickStart 向导**（推荐新手）和**手动配置**。

### 方式一：QuickStart 向导（推荐）

如果你是第一次配置 OpenClaw，最简单的方式是运行交互式向导：

```bash
openclaw quickstart
```

在渠道选择界面，选择 **Telegram (Bot API)**：

<figure>
  <img src="/images/integrations/telegram/4.png" alt="OpenClaw QuickStart 选择 Telegram 渠道" />
  <figcaption>QuickStart 向导会列出所有支持的聊天渠道，选择 Telegram (Bot API)</figcaption>
</figure>

然后粘贴你从 BotFather 获得的 Token：

<figure>
  <img src="/images/integrations/telegram/5.png" alt="输入 Telegram Bot Token" />
  <figcaption>将 BotFather 给你的 Token 粘贴到这里</figcaption>
</figure>

向导会自动完成剩余配置，包括生成配置文件和启动网关。
