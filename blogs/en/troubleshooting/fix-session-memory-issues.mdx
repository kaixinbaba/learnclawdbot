...
Session memory and context management are critical components of OpenClaw's architecture. When things go wrong—context windows overflow, important information gets lost during compaction, or sessions bloat uncontrollably—it can significantly impact your agent's performance and reliability. This guide will help you diagnose and fix these issues.
...
**Symptom**: `🧹 Auto-compaction complete` appears frequently, even in short sessions.
...
**DM scope options**:
- `main` (default): All DMs share one session—great for single-user continuity
...
├── memory/
│   ├── 2026-02-10.md     # Daily logs (append-only)
│   ├── 2026-02-11.md
│   └── 2026-02-13.md
...
- **Daily context and notes** → `memory/YYYY-MM-DD.md`
- **Never rely on "mental notes"** → Write everything important to disk
- **MEMORY.md loads only in main session** → Never in group contexts (security)
...
- Vector: Semantic matching ("Mac Studio gateway host" ≈ "machine running gateway")
