...
**⚠️ Critical Security Warning**: Your bot token is like a master password. Anyone with this token can control your bot. Never commit it to version control, share it publicly, or include it in screenshots. Store it securely using environment variables or encrypted configuration files.
...
- ✅ **Manage Messages** - Pin, delete messages (requires trust)
- ✅ **Manage Channels** - Create/edit channels (for automation)
- ✅ **Manage Roles** - Assign roles (high trust level)
- ✅ **Create Public Threads** - Start threads in channels
- ✅ **Create Private Threads** - Start private discussions
- ✅ **Manage Threads** - Archive, delete, manage threads
- ❌ **Administrator** - Avoid unless absolutely necessary
...
1. In your application, navigate to **"OAuth2"** → **"URL Generator"**
...
```bash
# Visit Discord Developer Portal → Your App → Bot → Privileged Gateway Intents
...
- ✅ Store tokens in encrypted vaults (AWS Secrets Manager, HashiCorp Vault)
...
- ❌ Store tokens in plaintext config files in shared locations
...
✅ **Bot Creation**: Developer Portal setup, intents, permissions  
✅ **Basic Configuration**: Token management, access control policies  
✅ **Roles & Permissions**: Guild-level, channel-level, user-level allowlists  
✅ **Threads & Forums**: Standard threads, forum channels, and the critical forum gotcha  
✅ **Multi-Server Setup**: Managing multiple guilds with different configurations  
✅ **Troubleshooting**: Real-world solutions to common issues  
...
Happy Discord-ing with OpenClaw! 🤖💬
